...
void scene_setup(scene_t *scene){
    // Sets up 4 boxes
    for (int i = 0; i < 4; i++){
        body_t *box = make_box(BOX_SPEC.x, BOX_SPEC.y, BOX_COLOR, 2);
    ...
    body_t *border1 = make_box(SPACING, BOARD_SPEC.y, BOX_COLOR, 1);
    ...
    // Sets up ball alley
    body_t *alley1 = make_box(SPACING, ALLEY_SPEC.y, (rgb_color_t) {1, 0, 1}, 1);
    body_t *alley2 = make_box(ALLEY_SPEC.x, SPACING, (rgb_color_t) {1, 0, 1}, 1);
    body_t *alley3 = make_box(SPACING, ALLEY_SPEC.y, (rgb_color_t) {1, 0, 1}, 1);
    ...
    body_t *cone2 = make_trapezoid(CONE_SPEC.x, CONE_SPEC.y, SPACING, -1, BOX_COLOR, 1);
    ...
    // Sets up losing area
    body_t *losing_area = make_box(LOSING_SPEC.x, LOSING_SPEC.y, BOX_COLOR, 2);
